...
class WsService {
  ...
  private isMessageValid<T extends WSMessage>(message: T, schema: ValidationSchema<T>): boolean {
  ...
  private handleUserJoined = async (message: WSMessageUserData) => {
    if (!this.isMessageValid(message, VALIDATION_SCHEMA.WS.USER_DATA)) {
      return;
    }
  ...
  private handleSetVoteValue = async (message: WSMessageSetVoteValue) => {
    if (!this.isMessageValid(message, VALIDATION_SCHEMA.WS.SET_VOTE_VALUE)) {
      return;
    }
  ...
  private handleSetVoteRoundTopic = async (message: WSMessageSetTopic) => {
    if (!this.isMessageValid(message, VALIDATION_SCHEMA.WS.SET_TOPIC)) {
      return;
    }
  ...
  private handleModifySessionUser = async (message: WSMessageModifyUser) => {
    if (!this.isMessageValid(message, VALIDATION_SCHEMA.WS.MODIFY_USER)) {
      return;
    }
  ...
  private handleUpdateSessionPermissions = async (message: WSMessageSessionPermissions) => {
    if (!this.isMessageValid(message, VALIDATION_SCHEMA.WS.SESSION_PERMISSIONS)) {
      return;
    }
  ...
  private handleUpdateUserPermissions = async (message: WSMessageUserPermissions) => {
    if (!this.isMessageValid(message, VALIDATION_SCHEMA.WS.USER_PERMISSIONS)) {
      return;
    }
